#!/bin/bash

# ###############################################################
# SECTION: INITIALIZE PATHS
# ###############################################################

declare DIR_BASH=$DIR/bash
declare DIR_BACKUPS=$DIR/ops/backups
declare DIR_CERTS=$DIR/ops/certs
declare DIR_CONFIG=$DIR/ops/config
declare DIR_DAEMONS=$DIR/ops/daemons
declare DIR_DATA=$DIR/ops/data
declare DIR_DOCS=$DIR/repos/prodiguer-docs
declare DIR_FRONTEND=$DIR/repos/prodiguer-fe
declare DIR_JOBS=$DIR/repos/prodiguer-server/jobs
declare DIR_LOGS=$DIR/ops/logs
declare DIR_PYTHON=$DIR/ops/venv/python
declare DIR_REPOS=$DIR/repos
declare DIR_TEMPLATES=$DIR/templates
declare DIR_SERVER=$DIR/repos/prodiguer-server
declare DIR_SERVER_TESTS=$DIR/repos/prodiguer-server/tests
declare DIR_TMP=$DIR/ops/tmp
declare DIR_VENV=$DIR/ops/venv
