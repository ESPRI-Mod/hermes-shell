...
main()
{
	...
	export PYTHONPATH=$PYTHONPATH:$PRODIGUER_DIR_SERVER_TESTS
	...
	if [ -z "$1" ]; then
		...
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS
	elif [ $1 = "all" ]; then
	    log "TESTS: executing all tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS
	...
	# Run main test(s).
		...
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_main.py

	# Run web service tests.
	elif [ $1 = "web" ]; then
	    log "TESTS: executing web service tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_api_utils_ep.py
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_api_utils_ws.py
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_api_metric.py
	elif [ $1 = "web-utils-ep" ]; then
	    log "TESTS: executing web service endpoint tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_web_utils_ep.py
	elif [ $1 = "web-utils-ws" ]; then
	    log "TESTS: executing web socket tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_web_utils_ws.py
	elif [ $1 = "web-sim-metrics" ]; then
	    log "TESTS: executing simulation metrics web service  tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_web_sim_metrics.py

	# Run CV tests.
	elif [ $1 = "cv" ]; then
	    log "TESTS: executing cv tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_cv.py

	# Run DB tests.
	elif [ $1 = "db" ]; then
	    log "TESTS: executing db tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_db_main.py
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_db_mq_hooks.py
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_db_types.py
	elif [ $1 = "db-main" ]; then
	...
	# Run MQ tests.
		...
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_mq.py

	# Run MQ tests.
	elif [ $1 = "mq-mock-producers" ]; then
	    log "TESTS: executing mq mock producer tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_mq_mock_producers.py

	# Run utility functions tests.
	elif [ $1 = "u" ]; then
	    log "TESTS: executing utils tests ..."
		nosetests -v -s $PRODIGUER_DIR_SERVER_TESTS/test_utils_convert.py
